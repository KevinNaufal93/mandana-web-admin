...
import { toNotificationsSearchString, type NotificationsQuery } from "@/lib/notifications/query";
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 pt-1">
      <p className="text-sm text-muted-foreground">
        Menampilkan {from}–{to} dari {meta.total} notifikasi
      </p>
        ...
        <nav aria-label="Navigasi halaman" className="flex items-center gap-1.5">
          {buildPageList(meta.page, meta.totalPages).map((item, i) =>
            item === "ellipsis" ? (
              ...
              <Link
                key={item}
                href={`${basePath}${toNotificationsSearchString(query, { page: item })}`}
