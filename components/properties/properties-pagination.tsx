...
import { toSearchString, type PropertyQuery } from "@/lib/properties/query";
...
export function PropertiesPagination({
  ...
  query: PropertyQuery;
  meta: PageMeta;
  /** Prefixed onto every page link — toSearchString() only returns the
   *  "?..." suffix (and nothing at all for page 1 with no other filters),
   *  which is not a valid standalone href for next/link. */
  basePath: string;
}) {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 pt-1">
      <p className="text-sm text-muted-foreground">
        Menampilkan {from}–{to} dari {meta.total} properti
      </p>
        ...
        <nav aria-label="Navigasi halaman" className="flex items-center gap-1.5">
          {buildPageList(meta.page, meta.totalPages).map((item, i) =>
            item === "ellipsis" ? (
              ...
              <Link
                key={item}
                href={`${basePath}${toSearchString(query, { page: item })}`}
