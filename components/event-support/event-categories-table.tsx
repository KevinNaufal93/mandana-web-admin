...
export function EventCategoriesTable({ rows, hasActiveFilters }: { rows: AdminEventCategory[]; hasActiveFilters: boolean }) {
  return (
    <Table>
      <TableHeader>
        <TableRow>
        ...
        {rows.length === 0 ? (
          <TableEmpty colSpan={COLUMN_COUNT}>
            {hasActiveFilters
              ...
              : "Belum ada kategori. Kategori yang ditambahkan akan muncul di sini."}
          </TableEmpty>
        ) : (
...
function CategoryRow({ row }: { row: AdminEventCategory }) {
  return (
    <TableRow>
      <TableCell>
        <Link href={`/event-support/categories/${row.id}`} className="flex items-center gap-3">
      ...
      <TableCell className="text-right text-sm text-primary">
        <Link href={`/event-support/items?categoryId=${row.id}`} className="hover:underline">
          {row.itemCount}
        </Link>
      </TableCell>
