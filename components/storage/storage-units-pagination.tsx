...
import { toUnitSearchString, type StorageUnitQuery } from "@/lib/storage/query";
import type { PageMeta } from "@/lib/api/server-client";

type PageItem = number | "ellipsis";

/** Copy of components/event-support/bookings-pagination.tsx, retyped to
 *  StorageUnitQuery — see that file's header comment for why this isn't a
 *  shared generic. */
...
export function StorageUnitsPagination({
  query,
  meta,
  basePath,
}: {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 pt-1">
      <p className="text-sm text-muted-foreground">
        Menampilkan {from}–{to} dari {meta.total} unit
      </p>
        ...
        <nav aria-label="Navigasi halaman" className="flex items-center gap-1.5">
          {buildPageList(meta.page, meta.totalPages).map((item, i) =>
            item === "ellipsis" ? (
              ...
              <Link
                key={item}
                href={`${basePath}${toUnitSearchString(query, { page: item })}`}
