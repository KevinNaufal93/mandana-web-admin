 ...
 * route change. Every session-dependent piece (<SidebarNav/>, which needs
 * the viewer's RBAC-granted modules; <NotificationBell/>; <UserMenu/>)
 * goes in its own Suspense instead, so the rest of the shell streams
 * immediately. getCurrentUser() is cache()'d, so all three plus whatever
 * the page itself calls share one GET /auth/me per request. proxy.ts
 * covers the request boundary; every page's own requireModule()/
 * requireAdmin() (see lib/auth/dal.ts) covers the render boundary.
 *
  ...
  return (
    <SidebarProvider defaultState={sidebarState}>
      <div className="flex min-h-svh">
        <Suspense fallback={<SidebarNavSkeleton collapsed={sidebarState === "collapsed"} />}>
          <SidebarNav />
        </Suspense>
